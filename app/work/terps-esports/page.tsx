...
import { ArrowLeft, Camera, Film, Palette, Mail, Users, Zap } from "lucide-react";
import Header from "@/components/Header";
import CustomCursor from "@/components/CustomCursor";
import Footer from "@/components/Footer";
import { cn } from "@/lib/utils";

/* ─── accent colour: UMD Red ─── */
const RED = "#E21833";
const GOLD = "#FFD200";
...
function StatCard({ value, label, icon }: { value: string; label: string; icon?: React.ReactNode }) {
  return (
    ...
    >
      {icon && <div className="mb-3 text-[#E21833]">{icon}</div>}
      <p className="font-manrope text-3xl font-bold text-text">{value}</p>
      <p className="mt-1 font-mono text-xs uppercase tracking-wide text-text-muted">{label}</p>
    </motion.div>
  );
}
        ...
        return (
          ...
          >
            <span className={cn(
              ...
              isActive ? "opacity-100 text-[#E21833] font-medium" : "text-text-muted"
            )}>
              {section.label}
            </span>
            <span className={cn(
...
export default function TerpsEsportsPage() {
  return (
    <div className="relative min-h-screen bg-[#FAFAFA] text-text selection:bg-red-100">
      <CustomCursor />
      ...
      {/* ══════════════════════════════════════════════
          HERO
      ══════════════════════════════════════════════ */}
      <section className="relative z-10 overflow-hidden pt-32 pb-0 md:pt-48">
        ...
        <div className="relative h-[480px] w-full md:h-[560px]">
          ...
          <div className="absolute bottom-0 left-0 right-0 px-6 pb-10 md:px-12">
            ...
            <div className="mb-6 flex items-center gap-3">
              <Link href="/#work" className="group flex items-center gap-2 font-mono text-xs text-white/50 transition-colors hover:text-white">
                <ArrowLeft size={12} className="transition-transform group-hover:-translate-x-0.5" />
                Back to Work
              </Link>
              <span className="h-1 w-1 rounded-full bg-white/30" />
              <span className="font-mono text-xs" style={{ color: RED }}>Creative Direction · Visual Branding</span>
            </div>
            ...
            <p className="mt-4 max-w-2xl font-mono text-sm leading-relaxed text-white/60 md:text-base">
          ...
          <div className="grid gap-8 border-b border-border/40 py-8 md:grid-cols-3">
            <div>
              ...
              <p className="font-mono text-sm leading-relaxed text-text-secondary">
                Brand identity · Game day graphics · Video direction & editing · Script writing · Green screen production · MailChimp newsletters · Social media content · Photo direction
              </p>
            </div>
            <div>
      ...
      <section id="te-entry" className="relative z-10 py-24 md:py-32">
        <div className="mx-auto max-w-[1280px] px-6 md:px-12">
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <MediaPop className="rounded-2xl overflow-hidden border border-border">
              <Image
        ...
        <div className="mx-auto max-w-[1280px] px-6 md:px-12">
          ...
          <Fade className="mb-12 max-w-2xl">
            ...
            <p className="mt-4 font-mono text-sm leading-relaxed text-text-secondary">
              Strict rules applied: <strong className="text-text">no AI-generated imagery</strong>, strict adherence to <a href="https://brand.umd.edu/colors" target="_blank" rel="noopener noreferrer" className="underline hover:text-text">UMD brand colors</a> (Terrapin Red, Maryland Gold, black & white), and everything built from scratch in Photoshop and Illustrator.
            </p>
          </Fade>
      ...
      ══════════════════════════════════════════════ */}
      <section id="te-coed" className="relative z-10 bg-[#0a0a0a] py-24 md:py-32">
        <div className="mx-auto max-w-[1280px] px-6 md:px-12">
          ...
          <div className="mb-16 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { step: "01", label: "The Script", desc: "Spent days writing the narrative arc — player introductions, character reveals, timing beats. Every second of the 90-second film was planned on paper first." },
              { step: "02", label: "The Storyboard", desc: "Scene-by-scene visual breakdown: camera angles, lighting moods, green screen placements, transition cues. Shared with the team to align before a single frame was shot." },
              { step: "03", label: "The Shoot", desc: "Set up green screens, positioned lighting rigs, directed each player through their scenes. Explained camera angles to videographer Seth, did mock runs with player AnPhu." },
              { step: "04", label: "The Edit", desc: "Post-production in Premiere Pro and After Effects: chroma key, character compositing, motion graphics, color grading, and sound design. Late nights in the office." },
            ].map((item, i) => (
              <Fade key={i} delay={i * 0.08}>
                <div className="rounded-xl border border-white/10 bg-white/5 p-6 h-full">
                  <span className="block font-mono text-[11px] mb-3" style={{ color: RED }}>{item.step}</span>
          ...
          {/* BTS images */}
          <Fade className="mb-6">
            <p className="mb-4 font-mono text-[10px] uppercase tracking-widest text-white/30">Behind the Scenes</p>
          </Fade>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { src: "/Esports/me explaining seth the camera man for the camera angles.jpeg", alt: "Explaining camera angles to Seth" },
              { src: "/Esports/Me and Camera man Seth in the picture me explaining to the players.jpeg", alt: "With Seth explaining to players" },
              { src: "/Esports/Me hustling days and nights editing the footage from the office post production.jpeg", alt: "Late night editing session" },
            ].map((img, i) => (
              <MediaPop key={i} delay={i * 0.08} className="overflow-hidden rounded-xl border border-white/10">
                <div className="relative aspect-[4/3] overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3">
                    <p className="font-mono text-[10px] text-white/70">{img.alt}</p>
                  </div>
                </div>
              </MediaPop>
            ))}
          ...
          <Fade className="mt-12">
            <p className="mb-4 font-mono text-[10px] uppercase tracking-widest text-white/30">The Full Co-Ed Valorant Intro</p>
            <div className="overflow-hidden rounded-2xl border border-white/10" style={{ aspectRatio: "16/9" }}>
              <video
                src="/Esports/Full Co-ed Val Intro Video.mp4"
                controls
                className="w-full h-full object-cover"
                poster="/Esports/NJC valorant thumbnail_1.5x.png"
              />
            </div>
          </Fade>
        </div>
        ...
        <div className="mx-auto max-w-[1280px] px-6 md:px-12">
            ...
            <div className="relative overflow-hidden rounded-2xl border border-border bg-white">
              <div className="grid lg:grid-cols-2 lg:items-center">
                <div className="p-8 md:p-12">
                  <span className="mb-3 block font-mono text-[10px] uppercase tracking-widest" style={{ color: RED }}>Most loved on social media</span>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              { src: "/Esports/Funtimewiththeteam.jpg", alt: "Fun time with the team" },
              { src: "/Esports/Teasertothe introvideo.mov", alt: "" },
            ].filter(img => !img.src.endsWith(".mov")).map((img, i) => (
      ...
      {/* ══════════════════════════════════════════════
          07 — NEWSLETTERS
      ══════════════════════════════════════════════ */}
      <section id="te-newsletters" className="relative z-10 py-24 md:py-32">
        <div className="mx-auto max-w-[1280px] px-6 md:px-12">
          <SectionHeading number="07" title="Newsletters — Reaching Beyond the Arena." />
      ...
      {/* ══════════════════════════════════════════════
          08 — REFLECTION
      ══════════════════════════════════════════════ */}
      <section id="te-reflection" className="relative z-10 bg-[#0a0a0a] py-24 md:py-32">
        <div className="mx-auto max-w-[1280px] px-6 md:px-12">
          <SectionHeading number="08" title="What This Project Gave Me." light />

          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 mb-16">
            {[
              { icon: <Film size={22} />, value: "1", label: "Full-length intro film directed & edited" },
